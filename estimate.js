/* [estimate.js Ver 1.74 - 인쇄 잔상 방지 및 계약서 줄바꿈 통합] */
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if(body && typeof data !== 'undefined') {
        data.forEach((sec, idx) => {
            ...
            let html = '';
            if (sec.key !== 'custom') html += `<div class="grid-row quote-grid no-print" style="background:#f8f9fa;"><label class="item-label"><input type="checkbox" onchange="toggleSec(${idx}, this)"><span class="checkmark"></span> <b>전체 선택</b></label></div>`;
            if(sec.isPaint) html += `<div class="grid-row quote-grid no-print" style="background:#fffbe6; border-bottom:2px solid #ffe58f;"><div style="font-weight:bold;">페인트 종류</div><div style="grid-column: span 3;"><select class="info-select" onchange="updatePaintPrice(${idx}, this)"><option value="water">수성 페인트 (12만)</option><option value="elastic">탄성 코트 (22만)</option><option value="ceramic">세라믹 코트 (30만)</option></select></div></div>`;
            sec.items.forEach(item => {
                html += `<div class="grid-row quote-grid item-line"><div class="col-name"><label class="item-label"><input type="checkbox" class="chk" data-name="${item.n}" onchange="updateSum()"><span class="checkmark"></span><span class="item-name-text">${item.n}</span></label></div><div class="col-center"><input type="number" class="input-num qty" value="1" oninput="updateSum()"></div><div class="col-center"><input type="number" class="input-num price" value="${item.p/10000}" oninput="updateSum()"></div><div class="col-right row-sum">0</div></div>`;
            });
...
function smartPrint() {
    ...
    const toast = document.getElementById('toast-msg'); if(toast) toast.classList.remove('show');
    ...
    document.querySelectorAll('.item-line').forEach(row => {
        ...
        if(!row.querySelector('.chk').checked) row.classList.add('hidden-print'); else row.classList.remove('hidden-print');
    });
    data.forEach((_, i) => {
        const cont = document.getElementById('cont-' + i);
        if(cont.querySelectorAll('.chk:checked').length === 0) cont.classList.add('hidden-print'); else cont.classList.remove('hidden-print');
    });
    window.print();
...
function addCustomRow(idx) {
    ...
    div.innerHTML = `<div style="display:flex; align-items:flex-start;"><label class="item-label" style="width:auto;"><input type="checkbox" class="chk" checked onchange="updateSum()"><span class="checkmark"></span></label><textarea class="info-input custom-name-input" style="padding:4px; font-size:13px; height:auto; flex:1; min-height:28px; line-height:1.2;" placeholder="항목명" oninput="updateSum()"></textarea><span class="custom-name-print"></span></div><div class="col-center"><input type="number" class="input-num qty" value="1" oninput="updateSum()"></div><div class="col-center"><input type="number" class="input-num price" value="0" oninput="updateSum()"></div><div class="col-right" style="display:flex; align-items:center; justify-content:flex-end;"><span class="row-sum" style="margin-right:5px;">0</span><button class="btn-del-row" onclick="this.closest('.grid-row').remove(); updateSum();">×</button></div>`;
    target.appendChild(div); updateSum();
}
...
function updateSum() {
    ...
    document.querySelectorAll('.grid-row.item-line').forEach(row => {
        ...
        if (chk && chk.checked) {
            ...
            total += sum; if(row.querySelector('.row-sum')) row.querySelector('.row-sum').innerText = sum.toLocaleString();
...
function switchToDetailed() {
    ...
    document.getElementById('btn-group-main').classList.remove('active'); document.getElementById('btn-group-sub').classList.add('active');
    window.scrollTo(0,0);
}

function backToGeneral() {
    document.getElementById('page-title').innerText = "견적서 작성";
    document.getElementById('view-general').style.display = 'block'; document.getElementById('view-detailed').style.display = 'none';
    document.getElementById('btn-group-main').classList.add('active'); document.getElementById('btn-group-sub').classList.remove('active');
}

function resetForm() { if(confirm('초기화하시겠습니까?')) location.reload(); }
function saveToLocal() { const t = document.getElementById('toast-msg'); t.classList.add('show'); setTimeout(() => { t.classList.remove('show'); }, 2500); }
...
function toggleSec(idx, master) { document.getElementById('c-' + idx).querySelectorAll('.chk').forEach(c => c.checked = master.checked); updateSum(); }
